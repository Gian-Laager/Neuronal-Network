...
TEST_F(Network, SetBias_WillBiasBeSetCorrectly)
{
    ...
    for (int i = 0; i < network.getLayer(0)->getSize(); i++)
        ASSERT_EQ(network.getLayer(0)->getNeuron(i)->getB(), firstBiases[i]);

    for (int i = 0; i < network.getLayer(1)->getSize(); i++)
        ASSERT_EQ(network.getLayer(1)->getNeuron(i)->getB(), secondBiases[i]);
}
...
TEST_F(Network, Callculate_WillTheValueBeCallculatedCorrectly)
{
    ...
    net.setWeights(0, std::vector<std::map<nn::abs::Neuron*, double>>{
            std::map<nn::abs::Neuron*, double>{{net.getLayer(1)->getNeuron(0).get(), 0.75},
                                               {net.getLayer(1)->getNeuron(1).get(), 0.5},
                                               {net.getLayer(1)->getNeuron(2).get(), 0.25}},

            std::map<nn::abs::Neuron*, double>{{net.getLayer(1)->getNeuron(0).get(), 0.3},
                                               {net.getLayer(1)->getNeuron(1).get(), 0.2},
                                               {net.getLayer(1)->getNeuron(2).get(), 0.1}}});

    net.setBias(1, std::vector<double>{0.8, 0.3, 0.2});
    net.setWeights(1, std::vector<std::map<nn::abs::Neuron*, double>>{
            std::map<nn::abs::Neuron*, double>{{net.getLayer(2)->getNeuron(0).get(), 0.2},
                                               {net.getLayer(2)->getNeuron(1).get(), 0.4}},

            std::map<nn::abs::Neuron*, double>{{net.getLayer(2)->getNeuron(0).get(), 0.6},
                                               {net.getLayer(2)->getNeuron(1).get(), 0.8}},

            std::map<nn::abs::Neuron*, double>{{net.getLayer(2)->getNeuron(0).get(), 1},
                                               {net.getLayer(2)->getNeuron(1).get(), 1.2}}});
